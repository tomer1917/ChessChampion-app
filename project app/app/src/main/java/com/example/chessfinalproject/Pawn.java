...
public class Pawn extends Soldier {
    public int moveDate = 0;
    public boolean doubleMoved = false;

    public Pawn(Bitmap imageBitmap, boolean whitePiece, int[] startingPosition, int[][] validMovement, boolean infiniteMovement,int[][] validAttack, boolean canRunOver,boolean canAttackOver, boolean infiniteAttack) {
    ...
    public void highlight(BoardTile[][] boardTileMatrix,boolean highlight){
        ...
        for (int i = 0; i <  validMovement.length; i++) {
            ...
            if (infiniteMovement) {
                ...
                //check double move on first move

                if (!whitePiece && rows >=3 && row == 1&&  boardTileMatrix[row + 2][col].currentSoldier == null &&  boardTileMatrix[row + 1][col].currentSoldier == null){
                    boardTileMatrix[row + 2][col].setHighlight(highlight);
                    boardTileMatrix[row + 2][col].invalidate();
                }
                else  if (whitePiece && rows >=3&& row == rows-2 &&  boardTileMatrix[row + -2][col].currentSoldier == null &&  boardTileMatrix[row + -1][col].currentSoldier == null){
                    boardTileMatrix[row - 2][col].setHighlight(highlight);
                    boardTileMatrix[row - 2][col].invalidate();
                }
            }
    ...
    public boolean IsValidMovement(BoardTile currentTile, BoardTile[][] boardTileMatrix) {
        ...
        for (int i = 0; i < validMovement.length; i++) {
            ...
            if (infiniteMovement) {
                ...
                if (Soldier.customChess){
                    if (!whitePiece && row == 1&& row + 2 == currentTile.row && col  == currentTile.col && boardTileMatrix[row + 2][col].currentSoldier == null &&  boardTileMatrix[row + 1][col].currentSoldier == null)
                        return true;
                    else  if (whitePiece && row == rows-2&& row + -2 == currentTile.row && col  == currentTile.col &&  boardTileMatrix[row + -2][col].currentSoldier == null &&  boardTileMatrix[row + -1][col].currentSoldier == null)
                        return true;
